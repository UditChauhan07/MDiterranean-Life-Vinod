...
const CustomDropdown = () => {
  const [selectedOption, setSelectedOption] = useState({img : '/package/PostCard.png', label : 'Visa / MasterCard / Amex / JCB'});
 ...
 const handleElectOption = () => {
  ...
  return (
    <div className={styles.Dropo} >
      <label>Select Payment  Method<span className={styles.HashRed}>*</span></label>
      <div className={styles.dropDown}>
        <div onClick={handleElectOption}>
          
          {selectedOption.img  && <img src={selectedOption.img} alt='card' />}
                    <label>{selectedOption.label}</label><span className={styles.arrowIcon}>	&#8735;</span>
        </div>
        {showOption && <ul>
